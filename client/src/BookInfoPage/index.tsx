...
const BookInfoPage: React.FC = (): JSX.Element => {
  ...
  const book: Book | undefined = Object.values(books).find(
    (b) => b.isbn === isbn
  );
  ...
  const deleteBook = async (): Promise<void> => {
    try {
      const { data: bookList } = await axios.get<Book>(
        `${baseUrl}/books/${isbn}`
      );
  ...
  return (
    <div className="App">
      ...
      <Divider hidden />
      <Button as={Link} to="/books" color="grey">
        Back
      </Button>
      <UpdateBookModal
        modalOpen={modalOpen}
        onSubmit={submitUpdateBook}
        onClose={closeModal}
        errMsg={error}
      />
      <Button
        color="blue"
        onClick={openModal}
        disabled={book?.uploader !== userInfo.id}
      >
        Update
      </Button>
      <Button
