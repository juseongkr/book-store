...
const DeleteUserForm: React.FC = (): JSX.Element => {
  ...
  const submitDeleteUser = async (values: NewUser): Promise<void> => {
    try {
      ...
      await axios.delete<User>(`${baseUrl}/auth/unregister`, {
        data: deleteUser,
      });
      dispatch({
        type: "SET_USER",
  ...
  const checkForm = (values: NewUser) => {
    ...
    if (
      ...
      !values.password?.match(pwRegex)
    ) {
      errors.password =
        "You must fill out password at least 8 characters including a number and a lowercase letter";
    }
