...
const MenuBar: React.FC = (): JSX.Element => {
  ...
  React.useEffect(() => {
    void (async () => {
      try {
        const { data: authUser } = await axios.get<UserInfo>(
          `${baseUrl}/auth/check`
        );
      ...
      {userInfo?.username ? (
        <Menu.Menu position="right">
          <Menu.Item
            ...
            active={actived === "info"}
            onClick={() =>
              dispatch({ type: "SET_ACTIVE", payload: ActiveItem.Info })
            }
            onMouseOver={() => InfoPage.preload()}
          >
            {userInfo?.name || userInfo.username.split("@")[0]}
          </Menu.Item>
          <Menu.Item
            as={Link}
            to="/"
        ...
        <Menu.Item
          ...
          name="login"
          position="right"
          active={actived === "login"}
          onClick={() =>
            dispatch({ type: "SET_ACTIVE", payload: ActiveItem.Login })
          }
